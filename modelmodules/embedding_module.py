...
class GraphEmbedding(EmbeddingModule):
    def __init__(self, node_features, edge_features, memory, neighbor_finder, time_encoder,
                 use_raw_node_features, train_node_embedding, dim_node_embedding, n_layers,
                 n_node_features, n_edge_features, n_time_features, embedding_dimension, device,
        ...
        if self.train_node_embedding:
            self.node_embeddings = torch.nn.Embedding(node_features.shape[0], dim_node_embedding)
    ...
    def compute_embedding(self, memory, source_nodes, timestamps, n_layers, n_neighbors=20, time_diffs=None,
        ...
        if self.train_node_embedding:
            source_node_embedding = self.node_embeddings(source_nodes_torch)
            if self.use_raw_node_features:
                source_node_features = torch.cat([source_node_features, source_node_embedding], dim=1)
            else:
                source_node_features = source_node_embedding
        ...
        if n_layers == 0:
            ...
            neighbors, edge_idxs, edge_times = \
                self.neighbor_finder.get_temporal_neighbor(source_nodes,
                                                           timestamps,
...
class GraphAttentionEmbedding(GraphEmbedding):
    def __init__(self, node_features, edge_features, memory, neighbor_finder, time_encoder,
        ...
        super(GraphAttentionEmbedding, self).__init__(node_features, edge_features, memory,
                                                      neighbor_finder, time_encoder,
                                                      use_raw_node_features, train_node_embedding, dim_node_embedding,
...
def get_embedding_module(module_type, node_features, edge_features, memory, neighbor_finder,
                         time_encoder, use_raw_node_features, train_node_embedding, dim_node_embedding,
    ...
    if module_type == "graph_attention":
        return GraphAttentionEmbedding(node_features=node_features,
                                       ...
                                       use_raw_node_features=use_raw_node_features,
                                       train_node_embedding=train_node_embedding,
